...
import { createAccount, createLevel, deleteLevel, getCurrentLevelName, getLevel, getLevelNames, isInsideLevelArea, login, runSaveStructure, saveLevel, sleep } from "utils";
async function signUpUI(player) {
    if (player.persistentId == "")
        return player.sendMessage(`You have to sign in to sign in.`);
    ...
    async function signUp() {
        ...
        if (rememberMe.getData())
            player.setDynamicProperty(`password`, pass);
        ...
        if (success.status == ServerStatusResponse.Success) {
            ...
            try {
                ...
                if (success.message == "Timed out")
                    realErr = `You aren't connected to the server! §7(/function connect)`;
                else {
                    const errData = JSON.parse(success.data)?.error;
                    const errMsg = errData?.message;
                    if (errMsg == "EMAIL_EXISTS")
                        realErr = `This account already exists! §7(You have an account)`;
                }
            }
            catch { }
...
function levelMainUI(player, levelName) {
    const maxLevels = 4;
    ...
    function allButtonSet(data) {
        for (const button of buttonData) {
            if (data.title != undefined)
                button.title.setData(data.title);
            if (data.cb != undefined)
                button.cb = data.cb;
            if (data.vis != undefined)
                button.vis.setData(data.vis);
            if (data.disabled != undefined)
                button.disabled.setData(data.disabled);
            if (data.spacerVis != undefined)
                button.spacerVis.setData(data.spacerVis);
            if (data.dividerVis != undefined)
                button.dividerVis.setData(data.dividerVis);
        }
    }
    form.show();
    ...
    function myLevelsForm() {
        ...
        for (let i = 0; i < maxLevels; i++) {
            ...
            if (i == maxLevels - 1)
                button.dividerVis.setData(true);
    ...
    function editLevelForm(levelName) {
        allButtonSet({ vis: false, disabled: false, dividerVis: false });
    ...
    function errorFunction(message, displayTicks = 30) {
    ...
    async function signIn(player) {
        ...
        if (!password) {
            return new Promise((resolve) => {
                ...
                loginButton.cb = () => {
                    if (toggle1.toggled.getData())
                        player.setDynamicProperty("password", textField1.text.getData());
                    resolve(loginInit(textField1.text.getData(), true));
                };
            });
        }
        async function loginInit(password, fromPrompt = false) {
            if (fromPrompt) {
                textField1.vis.setData(false);
                textField1.text.setData("");
    ...
    function createForm() {
        ...
        const nameCB = (text) => {
            ...
            if (!regex.test(text))
                validData = { message: "Invalid Characters!", valid: false };
            if (text.length <= 0)
                validData = { message: "Can't be 0 characters!", valid: false };
            if (text.length > 20)
                validData = { message: "Too Long", valid: false };
