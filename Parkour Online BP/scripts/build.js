...
import { isInside, runLoadStructure } from "utils";
const PARKOUR_CONFIG = {
    max: { x: 64, y: 128, z: 64 }
};
export function findAvailableDimension() {
    checkIfUsingDimensions();
    for (const dimension of dimensions) {
        if (dimension?.using != true)
            return dimension;
    }
}
...
export async function loadDimension(player, dim, level) {
...
world.beforeEvents.playerPlaceBlock.subscribe((data) => {
    ...
    if (!player.dimension.id.includes("parkour"))
        return;
    const max = PARKOUR_CONFIG.max;
