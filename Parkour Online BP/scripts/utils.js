import { BlockPermutation, BlockVolume, EnchantmentType, ItemLockMode, ItemStack, Player, system, world } from "@minecraft/server";
...
export function isInside(locationChecking, locations) {
    const blockVolume = new BlockVolume(locations[0], locations[1]);
    return blockVolume.isInside(locationChecking);
}
export function errorFunction(player, message, sound = true, soundName = "note.bass", color = "§c") {
    player.sendMessage(`§8[§dServer§8] ${color}${message}`);
    if (sound)
        player.playSound(soundName);
}
export function workedFunction(player, message, sound = true, soundName = "note.bell", color = "§a") {
    player.sendMessage(`§8[§dServer§8] ${color}${message}`);
    if (sound)
        player.playSound(soundName);
}
export function addItem(player, itemId, amount = 1, itemSlot, nameTag, lore, lockMode = ItemLockMode.none, destroyBlocks, equipmentSlot, enchantments) {
    const item = new ItemStack(itemId, amount);
    if (enchantments?.type != undefined)
        item.getComponent("enchantable")?.addEnchantment({ type: new EnchantmentType(enchantments.type), level: enchantments.level });
    item.lockMode = lockMode;
    item.nameTag = nameTag;
    item.setLore(lore);
    item.setCanDestroy(destroyBlocks);
    if (equipmentSlot == undefined && itemSlot == undefined)
        player.getComponent("inventory").container.addItem(item);
    else if (itemSlot != undefined)
        player.getComponent("inventory").container.setItem(itemSlot, item);
    else if (equipmentSlot != undefined)
        player.getComponent("equippable").setEquipment(equipmentSlot, item);
}
export function splitBytes(str, maxBytes = 32767) {
    const chunks = [];
    let currentChunk = "";
...
export function compressLZW(str) {
    const dict = {};
    for (let i = 0; i < 256; i++)
...
export function getCurrentLevelName(player) {
    return player.getDynamicProperty("currentLevel") ?? undefined;
}
export function createLevel(player, name, description) {
    const levelData = {
        creator: player.name,
        name,
        description
...
export function saveLevel(player, name, newLevelData) {
    ...
    if (newLevelData.newName != oldLevelData.name)
        deleteLevel(name);
    const lzw = compressLZW(JSON.stringify(levelData));
    const chunks = splitBytes(lzw);
    chunks.forEach((chunk, i) => {
...
export function deleteLevel(name) {
    const meta = world.getDynamicProperty(`parkourLevel|${name}|meta`);
    if (meta == undefined)
        return false;
...
export function isInsideLevelArea(player) {
...
export function getLevelNames() {
    return world.getDynamicPropertyIds()
        .filter(dp => dp.startsWith("parkourLevel") && dp.endsWith("meta")).map(dp => dp.split("parkourLevel|")[1].split("|meta")[0])
        .sort();
}
export function getLevel(name) {
    const levelMeta = world.getDynamicProperty(`parkourLevel|${name}|meta`);
    ...
    try {
        return JSON.parse(levelDataDecompressed);
    }
    catch { }
    ;
}
export function getSelectionBounds(pos1, pos2, add = 0) {
    return {
...
export function runSaveStructure(dimension, pos1, pos2, onProgress, onDone) {
    system.runJob(saveStructure(dimension, pos1, pos2, onProgress, onDone));
}
export function* saveStructure(dimension, pos1, pos2, onProgress, onDone) {
...
export function runLoadStructure(structure, dimension, location, onDone) {
    system.runJob(loadStructure(structure, dimension, location, onDone));
}
export function* loadStructure(structure, dimension, location, onDone) {
        ...
        for (let z = 0; z < structure.size.z; z++) {
            for (let x = 0; x < structure.size.x; x++) {
                if (currentRunCount === 0) {
                    if (rleIndex >= structure.blocks.length)
                        break;
...
async function signIn(player) {
    ...
    let password = player.getDynamicProperty(`password`);
    if (!password) {
        usingDefault = false;
...
export async function saveOnline(player, level) {
    try {
        const account = await signIn(player);
        if (!account)
            return player.sendMessage(`Failed Login`);
...
async function promptPassword(player) {
    let password = null;
    ...
    if (formValues[1])
        player.setDynamicProperty(`password`, password);
    return password;
}
